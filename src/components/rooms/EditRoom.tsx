...
import { RoomUpdate, updateRoom } from "../../redux/service/roomService";
...
export function EditRoomDialog({ room }: RoomCardProps) {
  ...
  const onSubmit = async (data: RoomUpdate) => {
    setIsSubmitting(true);

    try {
      // Update existing room
      if (room) {
        dispatch(updateRoom({ roomId: room._id, roomData: data }));
    ...
    <Dialog open={open} onOpenChange={(isOpen) => setOpen(isOpen)}>
      ...
      <DialogContent className="sm:max-w-[540px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the details for the room. Click save when you're done.
          </DialogDescription>
        </DialogHeader>
